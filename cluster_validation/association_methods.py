...
def radius_cut(richness_0, dist, cond=None):
    ...
    richness = richness_0
    radius_0 = r_richness_scaled(richness)
    #print(radius_0)
    richness_i =  np.sum(cond * (dist<=radius_0))

    #print(richness, richness_i)
    
    while richness != richness_i:
        richness = richness_i
        radius_i = r_richness_scaled(richness_i)      
        richness_i =  np.sum(cond * (dist<=radius_i))
...
def get_membership(cat_ref, cat, list_of_volume_match, truth_member_data, cluster_member_data, association_way):
    ...
    membership_fraction_of_associations = list_of_volume_match.copy()
    ...
    for i in range(len(cat_ref)): 
        ...
        membership_fraction_of_associations[i] = np.array(membership) / len(ref_member_id_list)

    return membership_of_associations, membership_fraction_of_associations


#_____________________________________________________
def membership_associations(ref_cat, cat, list_of_volume_match, truth_member_data, cluster_member_data, membership_fraction, association_way):
    
    """Perform membership association based on membership_fraction and return number and list of associations"""
  
    number_of_membership_match  = np.zeros(len(ref_cat))
    list_of_membership_match = np.copy(list_of_volume_match)
    ind = np.linspace(0, len(cat)-1, len(cat), dtype=int)

    membership_of_associations, membership_fraction_of_associations = get_membership(ref_cat, cat, list_of_volume_match, truth_member_data, cluster_member_data, association_way)
    
    
    for i in range(number_of_membership_match.size):
        
        if len(list_of_volume_match[i])>1:
        
            spatial_candidates = list_of_volume_match[i]
        
            membership_candidates = membership_fraction_of_associations[i]> membership_fraction
        
            number_of_membership_match[i] = np.sum(membership_candidates)
            list_of_membership_match[i]   = list_of_volume_match[i][membership_candidates]
    
    return number_of_membership_match, list_of_membership_match
...
def volume_match(truth_data, cluster_data, delta_zmax, r_max, r_max_type, method, cosmo, truth_member_data=None, cluster_member_data=None, delta_ovd=200, density_type="critical"):
    ...
    """
        ...
        :type density_type: string
         ...
         #results:        
        :return: two vectors containing the number of associations for each entry in the two ways, and one array containing the indices of unique associated objects in both catalogs (in a bijective way) following: [ind_RM,ind_DC2]
...
#_____________________________________________________
def membership_match(truth_data, cluster_data, membership_fraction, cosmo, truth_member_data=None, cluster_member_data=None):
    
    """
        Perform a membership association between DC2 and RM catalogs. 
 
        The association is computed in two ways (DC2=>RM and RM=>DC2), the intersection of which is bijective.
         #parameters:
        :param truth_data : the truth catalog
        :param membership_fraction : 
        :param cosmo: an astropy cosmology object
        :param truth_member_data: (optional, to be used if 'method' = 'membership') , catalog containing the membership info of the truth catalog, default to None
        :param cluster_member_data: (optional, to be used if 'method' = 'membership') , catalog containing the membership info of the detection catalog, default to None
        :param delta_ovd: (optional, to be used if 'r_max_type' = 'scaled') overdensity constrast to be used to compute the scaled radius, default to 200         
       ...
       :type membership_fraction : float
    ...
    ###compute searching distances-------------------------------------------------------------
    #arbitrary high searching radius to reduce computational time
    theta_max = search_distance(truth_data, cluster_data, 10., "fixed_dist", cosmo, "1w")

    ###search and store volume candidate------------------------------------------------------
    #arbitrary high searching volume to reduce computational time    
    number_of_volume_match_1w, list_of_volume_match_1w = \
    volume_associations(truth_data, cluster_data, theta_max, np.inf, "1w", coo_DC2, coo_RM)
    
    ###search and store membership candidate------------------------------------------------------  
    number_of_membership_match_1w, list_of_membership_match_1w = \
   membership_associations(truth_data, cluster_data, list_of_volume_match_1w, truth_member_data, cluster_member_data, membership_fraction, "1w")
    
    ###select only one match---------------------------------------------------------------------
    ind_DC2_match_1w, ind_RM_match_1w = \
    select_one_association(truth_data, number_of_membership_match_1w, list_of_membership_match_1w, 'membership', coo_DC2, coo_RM, cluster_data, truth_member_data, cluster_member_data, "1w")
    ...
    ###compute searching distances-------------------------------------------------------------
    #arbitrary high searching radius to reduce computational time
    theta_max = search_distance(truth_data, cluster_data, 10., "fixed_dist", cosmo, "2w")
    
    ###search and store volume candidate------------------------------------------------------
    #arbitrary high searching volume to reduce computational time    
    number_of_volume_match_2w, list_of_volume_match_2w = \
    volume_associations(truth_data, cluster_data, theta_max, np.inf, "2w", coo_DC2, coo_RM)

    ###search and store membership candidate------------------------------------------------------  
    number_of_membership_match_2w, list_of_membership_match_2w = \
   membership_associations(cluster_data, truth_data, list_of_volume_match_2w, truth_member_data, cluster_member_data, membership_fraction, "2w")
